...
export default defineNuxtModule<EmailModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    const templatesDir = resolvePath(nuxt.options.rootDir, options.templateDir!)
    ...
    const templatesFile = addTemplate({
      ...
      getContents: () => {
        ...
        const imports = templates
          .map((t, i) => `import t${i} from ${JSON.stringify(t.absPath.replace(/\\/g, '/'))}`)
          .join('\n')
    ...
    ;(nuxt.hook as (name: string, cb: NitroConfigHook) => void)('nitro:config', (nitro) => {
      nitro.alias ||= {}
