...
export class ConsoleProvider implements EmailProvider {
  ...
  async send(payload: NormalizedPayload): Promise<EmailResponse> {
    ...
    if (payload.cc?.length) console.log(`  CC:      ${payload.cc.join(', ')}`)
    if (payload.bcc?.length) console.log(`  BCC:     ${payload.bcc.join(', ')}`)
    console.log(`  Subject: ${payload.subject}`)
    if (payload.replyTo) console.log(`  ReplyTo: ${payload.replyTo}`)
    console.log(`  HTML:    ${payload.html.slice(0, 120)}${payload.html.length > 120 ? '...' : ''}`)
    if (payload.text) console.log(`  Text:    ${payload.text.slice(0, 120)}${payload.text.length > 120 ? '...' : ''}`)
