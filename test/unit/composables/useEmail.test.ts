...
import type {
	EmailResponse,
	NormalizedPayload,
} from '../../../src/runtime/types/index.js'
...
const { useEmail } =
	await import('../../../src/runtime/server/composables/useEmail.js')
...
describe('useEmail — validation', () => {
	...
	it('throws when `to` is missing', async () => {
		...
		await expect(
			sendEmail({ to: '', subject: 'Test', html: '<p>Hi</p>' }),
	...
	it('fails when the template does not exist', async () => {
		const { sendEmail } = useEmail()
		await expect(
			sendEmail({ to: 'a@b.com', subject: 'Test', template: 'missing' }),
